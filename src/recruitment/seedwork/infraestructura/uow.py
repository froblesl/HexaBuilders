...
class UnitOfWork(ABC):
    """
    Unit of Work pattern implementation for recruitment service.
    ...
    def get_entities_with_events(self) -> List[Any]:
        """Get all entities that have pending domain events."""
        return [
            entity for entity in self._entities.values()
            if hasattr(entity, 'get_domain_events') and entity.get_domain_events()
        ]
...
class Repository(ABC):
    ...
    def _register_entity(self, entity: Any) -> None:
        """Register an entity with the unit of work."""
        self._uow.register_entity(entity)
